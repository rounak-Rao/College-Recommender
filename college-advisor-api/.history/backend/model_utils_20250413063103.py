...
model_path = os.path.join('backend/models/debt_model.pkl')
...
earnings_df = pd.read_csv("backend/data/Most-Recent-Cohorts-Field-of-Study.csv", low_memory=False)
df_inst = pd.read_csv("backend/data/Most-Recent-Cohorts-Institution.csv", low_memory = False)
...
def evaluate_college(
    ...
    return {
        ...
        "final_score": round(final_scores[best_name], 2),
        "roi_score": best['roi_score'],
        "qol_score": best['qol_score'],
        "adjusted_debt": best['adjusted_debt'],
        "median_earnings": best['earnings'],
